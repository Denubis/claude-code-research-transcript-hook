...
# =============================================================================
# Test directory collision detection (silent-clobber regression)
# =============================================================================


class TestDirectoryCollision:
    """When distinct session UUIDs sanitise to the same directory name (typically
    because their first user messages are identical boilerplate envelopes like
    `<local-command-caveat>`), archive() must NOT silently share one directory —
    that was the silent-data-loss bug. Distinct sessions get distinct dirs via
    an auto-suffix derived from the session_id; same-UUID re-archive keeps using
    its existing directory (preserves the resumed-session/--last_size path)."""
                        ...
                        "content": [
                            {
                                ...
                                "text": (
                                    ...
                                    "were generated by the user while running "
    ...
    def test_distinct_sessions_get_distinct_directories(self, temp_dir):
        """Two distinct UUIDs with title-colliding first messages must end up in
        different archive directories. Before the fix, both wrote to one dir and
        the later one clobbered the earlier."""
        archive_dir = temp_dir / "archives"
        transcript_a = temp_dir / "session-aaaaaaaa.jsonl"
        transcript_b = temp_dir / "session-bbbbbbbb.jsonl"
        self._make_transcript(transcript_a)
        self._make_transcript(transcript_b)
        ...
        assert out_a != out_b, "distinct UUIDs must not share a directory"
        assert out_a.exists() and out_b.exists()

        meta_a = json.loads((out_a / "session.meta.json").read_text())
        meta_b = json.loads((out_b / "session.meta.json").read_text())
        assert meta_a["session"]["id"] == "aaaaaaaa-1111-2222-3333-444444444444"
        assert meta_b["session"]["id"] == "bbbbbbbb-5555-6666-7777-888888888888"

    def test_collision_emits_stderr_warning(self, temp_dir, capsys):
        """The collision case should be loud — not silent — so audits can find
        it. Emit a stderr warning naming both UUIDs and the suffix that was
        added."""
        archive_dir = temp_dir / "archives"
        transcript_a = temp_dir / "session-a.jsonl"
        transcript_b = temp_dir / "session-b.jsonl"
        self._make_transcript(transcript_a)
        self._make_transcript(transcript_b)
        ...
        capsys.readouterr()  # discard first-archive output
        archive(
            session_id="bbbbbbbb-5555-6666-7777-888888888888",
            transcript_path=transcript_b,
            archive_dir=archive_dir,
        ...
        captured = capsys.readouterr()
        assert "collision" in captured.err.lower() or "suffix" in captured.err.lower(), (
            f"expected stderr warning about collision, got: {captured.err!r}"
        )

    def test_same_session_reuses_directory(self, temp_dir, sample_transcript_content):
        """Re-archiving the SAME UUID (resume / --last_size path) must keep
        using the same directory — that's how resumed sessions and stitched
        clusters layer in. Only DIFFERENT session_ids trigger the suffix."""
        archive_dir = temp_dir / "archives"
        transcript = temp_dir / "session.jsonl"
        transcript.write_text(sample_transcript_content)

        out_a = archive(
            session_id="same-uuid-1234",
            transcript_path=transcript,
            archive_dir=archive_dir,
            quiet=True,
        )
        # Make the file look bigger so we don't hit the unchanged-skip path.
        transcript.write_text(sample_transcript_content + "\n")
        out_b = archive(
            session_id="same-uuid-1234",
            transcript_path=transcript,
        ...
        assert out_a == out_b, "same UUID must reuse its directory"


# =============================================================================
# Test manifest-pointer protection (curated-Three-Ps regression)
# =============================================================================


class TestManifestPointerProtection:
    """The previous bug: a hook-driven sweep re-archived a session that already
    had hand-curated Three Ps (set via /transcript), regenerated the title from
    content, created a fresh directory, and repointed the manifest at it.
    The old directory was left orphaned on disk and its Three Ps invisible.

    Protection: if archive() would repoint the manifest to a directory with
    EMPTY Three Ps while the existing target has NON-EMPTY Three Ps, refuse
    the repoint and emit a stderr warning. --force is the realistic trigger
    (without --force, the existing-dir branch reuses the curated directory)."""

    def _make_transcript(self, path: Path, content_marker: str) -> None:
        path.write_text(
            json.dumps(
                {
                    "type": "user",
                    ...
                    "message": {
                        "role": "user",
                        "content": [{"type": "text", "text": f"Real prompt: {content_marker}"}],
                    },
                }
            )
            + "\n"
    ...
    def test_force_does_not_clobber_curated_three_ps(self, temp_dir, capsys):
        """The realistic scenario: user curated Three Ps via /transcript; later
        a force-archive run (e.g. manual sweep) would otherwise replace the
        manifest pointer and orphan the curated dir."""
        archive_dir = temp_dir / "archives"
        transcript = temp_dir / "session.jsonl"
        self._make_transcript(transcript, "initial curated archive")

        curated = archive(
            session_id="ssss-1111-2222-3333-4444",
            transcript_path=transcript,
            archive_dir=archive_dir,
            three_ps={
                "prompt_summary": "researcher asked X",
                "process_summary": "used Y workflow",
                "provenance_summary": "part of Z project",
            },
            quiet=True,
        )
        assert curated is not None
        meta = json.loads((curated / "session.meta.json").read_text())
        assert meta["three_ps"]["prompt_summary"] == "researcher asked X"

        # Now simulate a sweep-style re-archive that would regenerate the title.
        # Change the file size so the unchanged-skip path is bypassed.
        self._make_transcript(transcript, "subsequent sweep with new content")
        ...
        repoint = archive(
            ...
            archive_dir=archive_dir,
            force=True,
            three_ps=None,  # sweep doesn't supply three_ps
            quiet=False,
        )

        # Curated dir must still be on disk with its Three Ps intact.
        assert curated.exists()
        post_meta = json.loads((curated / "session.meta.json").read_text())
        assert post_meta["three_ps"]["prompt_summary"] == "researcher asked X", (
            "curated Three Ps survived the sweep"
        )
        ...
        assert repoint == curated

        # Stderr warning so the audit trail is loud.
        captured = capsys.readouterr()
        assert "three" in captured.err.lower() or "curated" in captured.err.lower() or (
            "protect" in captured.err.lower()
        ), f"expected protection warning, got: {captured.err!r}"

    def test_three_ps_update_allowed(self, temp_dir):
        """When the incoming run DOES provide three_ps, the protection must
        not fire — that's the legitimate update path."""
        archive_dir = temp_dir / "archives"
        transcript = temp_dir / "session.jsonl"
        self._make_transcript(transcript, "initial")

        first = archive(
            session_id="zzzz-1111-2222-3333-4444",
            transcript_path=transcript,
            archive_dir=archive_dir,
            three_ps={
                "prompt_summary": "original",
                "process_summary": "original",
                "provenance_summary": "original",
            },
            quiet=True,
        )
        assert first is not None

        self._make_transcript(transcript, "updated")
        second = archive(
            session_id="zzzz-1111-2222-3333-4444",
            transcript_path=transcript,
            archive_dir=archive_dir,
            force=True,
            three_ps={
                "prompt_summary": "updated",
                "process_summary": "updated",
                "provenance_summary": "updated",
            },
            quiet=True,
        )
        assert second is not None
        # New three_ps replaced the old ones (no protection refusal).
        latest_meta = json.loads((second / "session.meta.json").read_text())
        assert latest_meta["three_ps"]["prompt_summary"] == "updated"
